...
import { DataWorksDashboard } from "@/pages/dataworks/dashboard"
import { DataWorksProvider } from "@/context/dataworks"
import { LoginPage } from "@/pages/dataworks/shell"

const NewSession = lazy(() => import("@/pages/new-session"))
const DataWorksConnections = lazy(() => import("@/pages/dataworks/connections"))
const DataWorksExplorer = lazy(() => import("@/pages/dataworks/explorer"))
const DataWorksJobs = lazy(() => import("@/pages/dataworks/jobs"))
const DataWorksSkills = lazy(() => import("@/pages/dataworks/skills"))
const DataWorksKnowledge = lazy(() => import("@/pages/dataworks/knowledge"))
const DataWorksAudit = lazy(() => import("@/pages/dataworks/audit"))
...
function DraftRoute() {
  ...
  const settings = useSettings()
  const tabs = useTabs()
  const explicit = search.draftId
  return (
    <Show when={tabs.ready()}>
      <Show
        when={explicit ? tabs.store.find((tab): tab is DraftTab => tab.type === "draft" && tab.draftID === explicit) : true}
        keyed
        ...
        {(draft) => (
          <Show
            ...
            fallback={
              <Navigate
                href={`/${base64Encode(
                  typeof draft === "object" && draft !== null && "directory" in draft
                    ? (draft as DraftTab).directory
                    : "",
                )}/session`}
              />
            }
          >
            <ResolvedDraftRoute draft={draft as DraftTab | true} />
          </Show>
        )}
      </Show>
    </Show>
...
function ResolvedDraftRoute(props: { draft: DraftTab | true }) {
  const global = useGlobal()
  // /new-session lives outside the per-directory SDKProvider. Directory and
  // server must come from the draft tab (created by tabs.newDraft). Do not call
  // useSDK() here — it throws "SDK context must be used within a context provider".
  const directory = createMemo(() => {
    if (typeof props.draft === "object" && props.draft !== null && "directory" in props.draft) {
      return (props.draft as DraftTab).directory
    }
    return ""
  })
  const conn = createMemo(() => {
    if (typeof props.draft === "object" && props.draft !== null && "server" in props.draft) {
      return global.servers.list().find((item) => ServerConnection.key(item) === (props.draft as DraftTab).server)
    }
    return global.servers.list()[0]
  })
        ...
        <ServerSyncProvider server={conn}>
          <ModelsProvider directory={directory}>
            <SDKProvider directory={directory()}>
              <DirectoryDataProvider directory={directory} server={() => (conn() ? ServerConnection.key(conn()!) : undefined)}>
                <DraftProviders>
                  <NewSession />
                </DraftProviders>
...
function Routes(props: { serverScoped?: JSX.Element }) {
  ...
  return (
    <>
      ...
      <Show when={settings.general.newLayoutDesigns()}>
        <Route path="/" component={DataWorksDashboard} />
        <Route path="/:dir/session/:id" component={NewLayoutLegacySessionRedirect} />
        <Route path="/server/:serverKey/session/:id" component={TargetSessionRoute} />
      </Show>
      <Route path="/new-session" component={DraftRoute} />
      <Route path="/login" component={LoginPage} />
      <Route path="/dataworks/connections" component={DataWorksConnections} />
      <Route path="/dataworks/explorer" component={DataWorksExplorer} />
      <Route path="/dataworks/jobs" component={DataWorksJobs} />
      <Route path="/dataworks/skills" component={DataWorksSkills} />
      <Route path="/dataworks/knowledge" component={DataWorksKnowledge} />
      <Route path="/dataworks/audit" component={DataWorksAudit} />
      <Route path="/dataworks" component={() => <Navigate href="/dataworks/connections" />} />
    </>
  )
}
