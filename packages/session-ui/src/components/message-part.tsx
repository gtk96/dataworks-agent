...
import { parseSqlResultView, sqlResultSubtitle } from "./sql-result"
...
export function getToolInfo(
  ...
  switch (tool) {
    ...
    case "dw_run_sql":
      return {
        icon: "console",
        title: i18n.t("ui.tool.dw_run_sql"),
        subtitle:
          typeof input.sql === "string"
            ? input.sql.slice(0, 80)
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    const sql = createMemo(() => (typeof props.input.sql === "string" ? props.input.sql : undefined))
    const subtitle = createMemo(() => {
      const current = view()
      if (current) return sqlResultSubtitle(current)
      if (typeof props.input.projectID === "number") return `project ${props.input.projectID}`
      return undefined
    })
    return (
      <SqlResultCard
