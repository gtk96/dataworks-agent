...
import { assertNotPrivatePathEffect } from "./private-path"
...
const ask = Effect.fn("ShellTool.ask")(function* (ctx: Tool.Context, scan: Scan, input: { command: string }) {
  // Mandatory private-path deny for every discovered path before permission prompts.
  for (const dir of scan.dirs) {
    yield* assertNotPrivatePathEffect(dir, "shell")
  }
  for (const file of scan.files) {
    yield* assertNotPrivatePathEffect(file, "shell")
  }
      ...
      for (const node of commands(root)) {
        ...
        if (cmd && (FILES.has(cmd) || (shellKind === "cmd" && CMD_FILES.has(cmd)))) {
          for (const arg of pathArgs(command, ps, shellKind === "cmd")) {
            ...
            // Always record every discovered path for private-path mandatory deny,
            // even when inside the project (private roots may be mounted inside).
