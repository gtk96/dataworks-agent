...
TASK_TEMPLATES: dict[str, dict[str, Any]] = {
        ...
        "steps": [
            {
                ...
                "params": ["table_name", "layer", "source_table"],
                "title": "Prepare source/Holo structure",
                "phase": "design",
            },
            {
                "tool": "create_mc_table",
                "params": ["table_name", "source_table"],
                "title": "Draft MaxCompute DDL",
                "phase": "design",
            },
            {
                "tool": "create_node",
                "params": ["table_name", "layer"],
                "title": "Draft DataWorks node",
                "phase": "orchestrate",
            },
            {
                "tool": "push_dml",
                "params": ["table_name", "source_table"],
                "title": "Draft DML and dependencies",
                "phase": "orchestrate",
                "risk": "medium",
            },
        ],
    ...
    "query_lineage": {
        ...
        "assumptions": [
            "Use Cookie fallback for downstream lineage when AK/SK metadata permission is unavailable."
        ],
        "steps": [
            {
                "tool": "query_lineage",
                "params": ["table_name", "depth"],
                "title": "Query lineage",
                "phase": "inspect",
            },
            {
    ...
    "ods_dwd_modeling": {
        ...
        "steps": [
            ...
            },
            {
                ...
                "title": "Plan ODS+DWD dependencies",
                "phase": "orchestrate",
            },
            {
                "tool": "validate_guardrails",
                "params": ["table_name", "dwd_table", "ods_table", "layer"],
                "title": "Validate guardrails",
                "phase": "guardrail",
            },
            {
                "tool": "recommend_next_actions",
                "params": ["goal", "table_name", "ods_table", "dwd_table"],
...
"forward_modeling": {
        ...
        "assumptions": [
            "Generate artifacts in dry-run mode before any DataWorks write.",
        ...
        "steps": [
            {
                ...
                "title": "Understand modeling goal",
                "phase": "understand",
            },
            {
                ...
                "title": "Generate modeling dry-run",
                "phase": "draft",
                "risk": "medium",
            },
            {
            ...
            },
            {
                ...
                "params": ["goal", "table_name", "source_table", "layer"],
    ...
    "diagnose_issue": {
        ...
        "steps": [
            ...
            },
            {
                ...
                "params": ["goal", "task_id", "table_name"],
    ...
    "metric_attribution": {
        ...
        "assumptions": [
            "Use semantic metadata and lineage; do not send production row data to LLM."
        ],
        "steps": [
            {
                "tool": "diagnose_metric_attribution",
                "params": ["goal", "metric_id", "table_name"],
                "title": "Diagnose metric attribution",
                "phase": "inspect",
            },
            {
                "tool": "recommend_next_actions",
        ...
        "steps": [
            {
                "tool": "validate_guardrails",
                "params": ["table_name", "layer"],
                "title": "Validate publish guardrails",
                "phase": "guardrail",
                "risk": "medium",
            },
            {
                "tool": "publish_gate_review",
                "params": ["goal", "table_name", "layer"],
                "title": "Prepare Publish Gate review",
                "phase": "guardrail",
                "risk": "high",
            },
            {
    ...
    "agent_workflow": {
        ...
        "assumptions": [
            "Plan and validate in dry-run/proposal mode first.",
            "AK/SK and Cookie fallback coexist by capability matrix.",
        ],
        "steps": [
            {
                "tool": "analyze_requirement",
    ...
    def _steps_from_template(self, template: dict[str, Any], intent: Intent) -> list[TaskStep]:
        ...
        for i, step_def in enumerate(template.get("steps", [])):
            params = {
                ...
                for p in step_def.get("params", [])
                if intent.params.get(p) is not None
            }
            steps.append(
                TaskStep(
                    step_id=f"step_{i}",
