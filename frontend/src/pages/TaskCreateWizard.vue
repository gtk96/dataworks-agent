  ...
  <div style="max-width: 900px; margin: 0 auto">
    ...
    <el-card v-if="step === 0">
      ...
      <el-form label-position="top">
        <el-form-item label="主表（MaxCompute）">
          ...
          >
            <el-option v-for="t in sourceTables" :key="t.table_name"
              :label="`${t.project}.${t.table_name}`"
              :value="`${t.project}.${t.table_name}`">
        ...
        <!-- DWD: 额外表 -->
        <template v-if="form.target_layer === 'DWD'">
    ...
    <el-card v-if="step === 1">
      ...
      <el-form label-position="top">
        <el-row :gutter="16">
          <el-col :span="8">
            <el-form-item label="数据层">
              <el-select v-model="form.target_layer">
                <el-option v-for="l in ['DWD','DWS','DMR','DIM']" :key="l" :value="l" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-form-item label="分区键">
          <el-checkbox-group v-model="form.partition_keys">
            <el-checkbox value="dt" label="dt (天)" />
            <el-checkbox value="hh" label="hh (小时)" />
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-card v-if="step === 3">
      ...
      <div v-else-if="preview">
        ...
        <template v-if="form.target_layer === 'DWD' && preview.dml">
          <h4>DML</h4>
    ...
    <el-card v-if="step === 4">
      ...
      <div v-else>
        <el-descriptions border :column="2">
          ...
          <el-descriptions-item label="目标层">{{ form.target_layer }}</el-descriptions-item>
...
function buildBody() {
  const body: any = { ...form.value, schedule_config: schedule.value }
  if (form.value.target_layer === 'DWD') {
    ...
    body.dwd_metadata = {
      ...
      joins: dwd.value.extra_sources.filter(s => s.table_name).map(s => ({
        ...
        right_table_name: s.table_name,
        right_alias: s.alias,
        on_condition: s.on_condition,
      })),
      field_mappings: dwd.value.field_mappings.filter(f => f.target_field).map(f => ({
        source_alias: mainAlias,
        source_field_name: f.source_field,
        target_field_name: f.target_field,
        transform_sql: f.transform || null,
      })),
